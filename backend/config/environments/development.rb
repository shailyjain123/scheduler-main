...
Rails.application.configure do
  ...
  # Surface delivery problems during local development.
  config.action_mailer.raise_delivery_errors = true

  # Make template changes take effect immediately.
  config.action_mailer.perform_caching = false

  # Use environment-driven host/port for mailer links.
  config.action_mailer.default_url_options = {
    host: ENV.fetch("BACKEND_HOST"),
    port: ENV.fetch("BACKEND_PORT", 3000).to_i
  }
  ...
  config.action_cable.allowed_request_origins = [ frontend_url, /http:\/\/localhost:\d+/ ]
